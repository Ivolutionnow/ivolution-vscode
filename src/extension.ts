...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Ivolution-autocompleter-plugin is now active!');
	...
	let provider = vscode.languages.registerCompletionItemProvider({ language: 'python'}, {
		...
		async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
			...
			const apiUrl : any = vscode.workspace.getConfiguration('ivolution-autocompleter-plugin').get('apiUrl');
			...
			try {
				...
				const items = data.result.map((suggestion: string) =>{
					...
					item.detail = "Ivolution Autocompleter";
				...
				console.error(err); 
				vscode.window.showInformationMessage('Ivolution Autocompleter - ' + err + ' - Something went wrong while connecting to service: '
				 + apiUrl);
			}
			return[];
		}
